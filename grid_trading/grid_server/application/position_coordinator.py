...
from ..domain.grid import round_down
...
class PositionCoordinator:
    ...
    def reconcile(
        ...
        configs = {
            ...
            if config.market_type == self.market_type
        }
        ...
        self.store.delete_position_pools_for_market_except(
            self.market_type,
            pool_keys,
        )
    ...
    def _reconcile_pool(
        ...
        if position_qty < current_logical_qty:
            previous_pool = self.store.get_position_pool(
                symbol,
                position_side,
                self.market_type,
            )
            shortage_confirmed = bool(
                previous_pool is not None
                and previous_pool["status"] == "shortage_pending"
                and Decimal(previous_pool["actual_qty"]) == position_qty
                and Decimal(previous_pool["logical_qty"]) == current_logical_qty
            )
            if not shortage_confirmed:
                # Binance order and position endpoints can briefly disagree
                # after a fill or automatic order cancellation. Rewriting Cell
                # ownership from one such snapshot permanently turns the
                # recovered quantity into an unassigned position. Preserve all
                # Cell ownership until a second independent reconciliation sees
                # the exact same aggregate shortage.
                self._save_pool(
                    symbol,
                    position_side,
                    position_qty,
                    current_logical_qty,
                    external_reserved,
                    status_override="shortage_pending",
                )
                return
