...
class SQLiteStore:
        ...
        with self.connection() as conn:
            conn.execute(
                ...
                INSERT INTO strategies (
                    strategy_id, symbol, mode, anchor_price, grid_ratio, grid_count,
                    order_usdt, leverage, poll_interval_sec, move_grid, status,
    ...
    def update_draft(self, config: StrategyConfig) -> None:
        ...
        with self.connection() as conn:
            ...
            conn.execute(
                """
                UPDATE strategies SET symbol=?, mode=?, anchor_price=?, grid_ratio=?, grid_count=?,
                    order_usdt=?, leverage=?, poll_interval_sec=?, move_grid=?, updated_at=?
                WHERE strategy_id=?
                """,
                (
                    config.symbol.strip().upper(),
        ...
        with self.connection() as conn:
            conn.execute(
                ...
                INSERT INTO position_pools(
                    symbol, position_side, actual_qty, logical_qty,
    ...
    def delete_position_pools_except(self, keys: set[tuple[str, str]]) -> None:
        with self.connection() as conn:
            if not keys:
            ...
            conn.execute(
                f"DELETE FROM position_pools WHERE (symbol, position_side) NOT IN ({placeholders})",
                params,
            )
